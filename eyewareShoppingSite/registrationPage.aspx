<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="registrationPage.aspx.cs" Inherits="eyewareShoppingSite.registrationPage" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title></title>
    <style>
        img{
            margin-top: -75px;
            margin-left: 750px;
            width: 325px;
            height: 275px;
        }
        .registration{
            background-color: whitesmoke;
            border: groove;
            border-color:darkblue;
            position: relative;
            margin-left: 600px;
            margin-top: -70px;
            width: 650px;
            height: 820px;
            border-radius:20px;
            text-align: start;
        }
        #heading1{
            color: darkblue;
            font-size: 40px;
            position: relative;
            left: 50px;
            font-style: italic;
            font-family: ui-monospace;
        }
        .TextBox1{
            border-radius: 20px;
            border: 1px solid #609;   
            width: 525px;
            height: 40px;
            position: relative;
            top: -15px;
            left: 50px;
            font-size: 20px;
            margin-top: 15px;
        }
        .referal{
            font-size:25px;
            position:relative;
            left:50px;
        }
        #Checkbox1{
            position: relative;
            left: -475px;
            top: 70px;
            width: 23px;
            height: 22px;
        }
        #label1{
            position: relative;
            left: 100px;
            top: 25px;
            font-size: 29px;
            color: darkblue;
        }
        #policies{
            color: darkblue;
            position: relative;
            top: 25px;
            left: 50px;
            font-size: 22px;
        }
        #Button1{
            background-color:darkgray;
            border-radius: 15px;
            border:1px solid #609;
            width: 525px;
            height: 50px;
            position: relative;
            top: 10px;
            left: 50px;
            font-size: 20px;
            margin-top: 15px;
            color: darkblue;
            cursor:pointer;
        }
        #login{
            color: darkblue;
            position: relative;
            top: 25px;
            left: 175px;
            font-size: 25px;
        }
        #Button2{
            border-radius: 15px;
            border:1px solid whitesmoke;
            position: relative;
            left: 623px;
            cursor:progress;
            
        }
        #whatsapp{
            position: relative;
            right: -252px;
            width: 40px;
            height: 40px;
            top: -220px;
        }
    </style>
</head>
<body>
    <form id="form1" runat="server">
        <div>
            <a href="https://logos-world.net/wp-content/uploads/2023/01/Lenskart-Logo.jpg">
              <img alt="problem in image" src="https://logos-world.net/wp-content/uploads/2023/01/Lenskart-Logo.jpg" class="auto-style9" />
            </a> 
        </div>
        <div class="registration">
            <asp:Button ID="Button2" runat="server" Text="X" OnClick="Button2_Click" />
            <h1 id="heading1">Create an Account </h1>
             <asp:TextBox class = "TextBox1" placeholder="  First Name* " runat="server" ID="fnameBox" autocomplete="off"></asp:TextBox>
             <asp:TextBox class = "TextBox1" placeholder="  Last Name " runat="server" ID="lnameBox" autocomplete="off"></asp:TextBox>
             <asp:TextBox class = "TextBox1" placeholder="  +91 Mobile* " runat="server" ID="numberBox" autocomplete="off"></asp:TextBox>
             <asp:TextBox class = "TextBox1" placeholder="  Email* " runat="server" ID="mailBox" autocomplete="off"></asp:TextBox>
             <asp:TextBox class = "TextBox1" placeholder="  Password* " runat="server" TextMode="Password" OnTextChanged="Unnamed5_TextChanged" ID="passwordBox"></asp:TextBox>
             <p class="referal"><a href="">Got a referral Code?</a>( Optional )</p>
            <asp:TextBox class = "TextBox1" placeholder="  Referal code (Optional) " runat="server" ID="referalBox" autocomplete="off"></asp:TextBox>
             <input id="Checkbox1" type="checkbox" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
             <label id="label1"> Get updates on Whatsapp</label>
            <p id ="policies">By creating this account, you agree to our <a href=""> Privacy Policy</a></p>
            <asp:Button ID="Button1" runat="server" Text="Create an Account " OnClick="Button1_Click" />
            <p id="login">Have an account? <a href="https://localhost:44320/loginPage.aspx"> Sign In</a></p>
        </div>
        <img alt="error" id="whatsapp" src="data:image/jpeg;base64," />
    </form>
    
</body>
</html>
